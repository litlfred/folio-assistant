...
| R11 | **Upstream asks** — four, itemised in [§8](ig-incremental-build.md#8-upstream--the-specific-asks-to-the-ig-publisher-and-the-validator) and stated as the smallest code change each in [§8.4](ig-incremental-build.md#84-the-smallest-change-per-ask), none folio-specific: **A, the AST** (serialise what the publisher already computes: per-resource records, the dependency graph, the toolchain, what was rendered where); **B**, differential builds (carry the skipped files' validation messages across a differential build and report the mode — no state directory, no temp flag, per the investigation in [§8.5](ig-incremental-build.md#85-what-the-investigation-found-and-what-is-therefore-not-asked)); **C**, a render-only `-no-jekyll` mode with the Simplifier path as precedent; **D**, a `/status` endpoint on the validator service that already serves validate, snapshot, narrative and load-IG | the publisher wants to be agnostic about its consumers, and the AST is agnostic by construction — it asks for nothing to be done differently, only for what is done to be written down ([§8.1](ig-incremental-build.md#81-already-there--use-do-not-ask), [§8.2](ig-incremental-build.md#82-the-asks)) | the publisher and the validator; A unblocks R7/R8, B makes `-rapido` from restored `txCache/` + `temp/` exact, C unblocks R9 | HL7 (Zulip) | 5 |

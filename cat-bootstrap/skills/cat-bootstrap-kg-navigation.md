...
1. **An instance declares the directories it scans**, in `<name>.json` at its
...
1. **Read the root `<name>.json`.** If there is none, this repository is not
...
|---|---|---|
| no `<name>.json` | not an instance yet | this is cat-bootstrap's case — continue |
| `<name>.json` that will not parse | an instance asserting something broken | **stop and say so**; do not fall back |

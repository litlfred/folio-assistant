---
name: cat-bootstrap-graph-publication
description: >
  Where cat-bootstrap's graph is published, why its `@id` must equal exactly
  that path, why a `.json` alias sits beside the canonical `.jsonld`, and why
  the document is NOT committed any more. The failure this skill exists to stop
  is `blv9` — a self-describing artefact whose own `@id` dereferenced to
  nothing, for months, while the generator worked perfectly.

  Prefixed `cat-bootstrap-` for the same resolver reason as its siblings here.
consulted: true
---

# Publishing cat-bootstrap's graph

## One path, named in three places, and they must agree

    @id            <base>/cat-bootstrap/cat-bootstrap.jsonld
    build writes   ./_site/cat-bootstrap/cat-bootstrap.jsonld
    served at      <base>/cat-bootstrap/cat-bootstrap.jsonld
...
That is not hypothetical. The site build published `cat-bootstrap/ns.jsonld` —
...
`scripts/tests/cat-bootstrap-graph.test.ts` matches on the `--out` path in
...
So `<base>/cat-bootstrap/cat-bootstrap.jsonld` is not a path somebody chose for
...
Until 2026-09-20 `cat-bootstrap/cat-bootstrap.jsonld` was a tracked file with a
...
as telling them to load it. **No prose file under `cat-bootstrap/` mentions
this document at all.** The README sends that reader to
`workflows/initialize-harness.bpmn` and
`skills/cat-bootstrap-kg-navigation.md`. The pair of tests defending
...
`bun run cat-bootstrap:graph` writes it — and that is fine.
...
- [`cat-bootstrap-graph-emission`](cat-bootstrap-graph-emission.md) — what the

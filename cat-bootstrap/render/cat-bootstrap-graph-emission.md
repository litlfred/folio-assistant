---
name: cat-bootstrap-graph-emission
description: >
  How cat-bootstrap emits its OWN graph as `.jsonld`/`.json`, and why that
  emission is the one obligation the layer cannot trade away. cat-bootstrap is
  ...
  *"is its existence"*. A layer that cannot emit its own graph has not shown it
  ...
  Prefixed `cat-bootstrap-` for the reason `cat-bootstrap-kg-navigation` is:
  both resolvers prefer `skills/folio-core/` on a bare name, and a cold agent
  served a harness-assuming body is the exact failure cat-bootstrap exists to
  prevent.
consulted: true
---

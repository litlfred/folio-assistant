...
| | |
|---|---|
| **Harness** | what a repository can be made into — `cat-bootstrap` itself, or a derivative such as `cat-harness`, `folio-assistant`, `smart-guidelines`. Declared by a [`cat-bootstrap.json`](cat-bootstrap.json). |
...
[`skills/cat-bootstrap-kg-navigation.md`](skills/cat-bootstrap-kg-navigation.md).
...
`<name>` is the `name` in that harness's `cat-bootstrap.json`. It is the same path
...
| **FR-5** | A repository whose root already carries a `cat-bootstrap.json` is **logged and ended**, never re-initialised. |
...
**A repository whose root carries a `cat-bootstrap.json` has been initialized**,
...
| [`cat-bootstrap.json`](cat-bootstrap.json) | this instance's own declaration: which directories it holds and what kind of graph each is |
| **workflows** | |
...
| **skills** | |
| [`skills/cat-bootstrap-kg-navigation.md`](skills/cat-bootstrap-kg-navigation.md) | how to open anything here, assuming nothing |
...
| **render** | |
| [`render/cat-bootstrap-graph-emission.md`](render/cat-bootstrap-graph-emission.md) | emitting this instance's own graph |
| [`render/cat-bootstrap-graph-publication.md`](render/cat-bootstrap-graph-publication.md) | publishing it |

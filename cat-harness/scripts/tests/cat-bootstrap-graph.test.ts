/**
 * The cat-bootstrap graph is committed, current, and a pure function of its inputs.
 *
 * @module scripts/tests/cat-bootstrap-graph.test
 */
...
describe("the document is published where it says it is", () => {
  /**
   * These two replaced "`cat-bootstrap.jsonld` is committed" and "it is current",
   ...
   * clone. **No prose file under `cat-bootstrap/` mentions this document** — the
   * README sends that reader to `workflows/initialize-harness.bpmn` and
   * `skills/cat-bootstrap-kg-navigation.md`. So the pair defended an instruction
   ...
   * `cat-bootstrap/ns.jsonld` — the NAMESPACE document — and never this one.
   *
  ...
  test("its `@id` is the URL the site build writes it to", async () => {
    ...
    // The build writes `./_site/cat-bootstrap/cat-bootstrap.jsonld`, and `_site/` is
    ...
    expect(id.endsWith("/cat-bootstrap/cat-bootstrap.jsonld")).toBe(true);
  });
  ...
  test("the site build actually writes it, at that path", () => {
    ...
    expect(wf).toContain("gen-cat-bootstrap-graph.ts --out \"./_site/cat-bootstrap/cat-bootstrap.jsonld\"");
  });
  ...
  test("it is NOT committed — it is a build artefact now", () => {
    ...
    // locally, since `bun run cat-bootstrap:graph` writes it; what must not
    // exist is a TRACKED copy.
    const tracked = execFileSync("git", ["ls-files", "--", "cat-bootstrap/cat-bootstrap.jsonld"], {
...
describe("pure, because committed-and-gated demands it", () => {
  ...
  test("the graph is ORDERED, so two machines agree byte for byte", async () => {
    ...
    // repository's. The committed file held skills as `cat-bootstrap-kg-
...
describe("what it contains, and what it admits it did not look at", () => {
  test("every skill cat-bootstrap holds is in the graph", async () => {
  ...
  test("cat-bootstrap publishes only the graph kinds it DECLARES", async () => {
    // Bean `3jj9`. `collectGraphKinds` emitted the UNIVERSAL registry into
    // every instance, so cat-bootstrap — whose premise is that it knows nothing
  ...
  test("the instance-bound collectors are named as NOT looked for", async () => {
    // "cat-bootstrap has no tools" and "tools were never looked for" are different
  ...
  test("cat-bootstrap's process IS in the graph, with its flows and lanes", async () => {
    ...
    // `cat-bootstrap/`, which had no `workflows/` when it was written. #413 gave
    // cat-bootstrap its decision tree and the assertion inverted: the directory is
...
function idsOfType(doc: Record<string, unknown>, type: string): string[] {
  ...
  return graph
    .filter((n) => {
      ...
      // `@type` is the full minted IRI — `<base>/cat-bootstrap/ns#Skill` — so the
...
/**
 * The skill bodies actually sitting in `cat-bootstrap/skills/`.
 *
 ...
 * stops finding `cat-bootstrap/skills/` exports an empty section and reports a
...
/** The processes actually drawn in `cat-bootstrap/workflows/`, by their BPMN id. */

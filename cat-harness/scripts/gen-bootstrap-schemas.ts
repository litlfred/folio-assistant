#!/usr/bin/env bun
/**
 * Generate `cat-bootstrap`'s JSON Schema from `cat-bootstrap-tools`' Zod.
 *
 ...
 *
 * `cat-bootstrap/README.md` promises an Initiator **no harness, no server, no
 ...
 * executable code. So the Zod lives in `cat-bootstrap-tools/` and the JSON
 * lands in `cat-bootstrap/`, which knows nothing of how it got there — owner,
 ...
 * `cat-bootstrap-tools/schemas/discussion.test.ts` proves it with documents
 * that must fail.
 *
 * Usage:
...
} from "../../cat-bootstrap-tools/schemas/discussion.ts";

const ROOT = join(import.meta.dir, "..", "..");
const check = process.argv.includes("--check");

/**
 * Where `cat-bootstrap` actually is, resolved from its own declaration.
 *
 * The INSTANCE is the part that can move — it is discovered by declared name
 * rather than assumed to sit at `cat-bootstrap/`, so a relocation moves the
 ...
 *
 * `skills/` cannot be resolved as a graph here in any case: `cat-bootstrap`
...
function bootstrapRoot(): string {
  for (const root of instanceRootsIn(ROOT)) {
    try {
      if (readDeclaration(root)?.name === "cat-bootstrap") return root;
    } catch {
  ...
  throw new Error(
    "no instance declares itself `cat-bootstrap` — cannot place the generated schemas. " +
...
const TARGETS = [
  {
    ...
    id: "https://litlfred.github.io/folio-assistant/cat-bootstrap/skills/discussion/input.schema.json",

#!/usr/bin/env bun
/**
 * Generate `bootstrap`'s JSON Schema from the Zod in `cat-harness/schemas/`.
 *
 ...
 * executable code. So the Zod lives in `cat-harness/schemas/` and the
 * JSON lands in `bootstrap/`, which knows nothing of how it got there — owner,
 * 2026-09-20: *"bootstrap should not know zod at all (it is just generated by
 * it in main and ghpages rendering pipeline)"*.
 *
 * ## Zod is one authoring tool; the output is the contract
 *
 * Owner, 2026-09-24: *"Zod - these are authoring tools for the KG, different
 * tools can/will be used, but the core KG should be unchanged."* The Zod sat
 * in a sibling instance, `bootstrap-tools`, until that day, and moved into
 * cat-harness with the other validators. What must not change with the tool
 * is what this writes: bootstrap's `schemas/*.schema.json`, byte for byte. A
 * different authoring tool (another language, a hand-written schema) is
 * judged the same way, by `bootstrap:schemas:check`.
 *
...
if (check && stale) {
  console.error(`\n${stale} bootstrap schema(s) stale — run \`bun run bootstrap:schemas\`.`);
  process.exit(1);
}
console.log(
  check

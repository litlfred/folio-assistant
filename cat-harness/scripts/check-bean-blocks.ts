...
if (import.meta.main) {
  ...
  const bad = found.filter((b) => b.verdict === "incomplete");
  ...
  console.log(
    `Bean blocks (${ok.length} structured and complete, ${bad.length} incomplete, ` +
  ...
  if (found.length === 0 && relation.length === 0) console.log("  ✓ no bean declares a block");
  process.exit(0);
}

...
import { orphanSubjectPages } from "./gen-schema-viz.ts";
 ...
 * generator's call.
 *
 * **It stays a denylist, and bean `y90d` asked whether it should** — an
 * allowlist derived from the declaration was the alternative. It stays
 * because the two questions turned out to be different ones. This list
 * governs WRITING: which route this generator refuses to publish into, where
 * being wrong means a page lands inside Jekyll's machinery. Pruning no longer
 * consults it at all — a directory is a candidate because its `index.html`
 * names itself, so an id missing from this list can no longer cause a
 * deletion, which was the incompleteness that made a denylist worrying.
 */
...
/**
 * How a dashboard says WHICH GRAPH it is for, in its own bytes.
 *
 * Bean `y90d`. Until this existed, pruning here could only have meant
 * "a directory under the site root that is not a declared graph" — and the
 * site root also holds the docs pages, `assets/`, and everything
 * just-the-docs emits, so that rule would have classified the whole site as
 * this generator's to delete. `deletion-requires-confirmation` forbids
 * exactly that assumption, which is why `y90d` was opened rather than
 * `ankg`'s fix being copied across.
 *
 * With the marker the question stops being *where does this sit* and becomes
 * *what does this file say it is*, and the site root stops mattering: a
 * neighbour that does not carry the marker is never a candidate, whatever it
 * is called.
 *
 * A `<meta>` rather than `gen-schema-viz`'s `var SCOPE` line because a
 * dashboard carries no inline script to put it in. Same rule, page-shaped.
 */
const GRAPH_META = /<meta name="fa-state-graph" content="([^"]*)">/;

/** Read a dashboard's own claim about which graph it serves. */
export function dashboardOwner(html: string): string | undefined {
  return GRAPH_META.exec(html)?.[1];
}
 ...
 *
 * `scope` is REQUIRED, and that is the point: every page this generator
 * writes goes through here, so the marker cannot be forgotten on one of them
 * — and a page without it is, correctly, not prunable.
 */
function page(opts: { title: string; scope: string; metas: string[]; body: string }): string {
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
<meta name="fa-state-graph" content="${esc(opts.scope)}">
<!--
...
/**
 * Dashboards for graphs the declaration no longer describes — found, and
 * removed or reported.
 *
 * **Bean `y90d`.** `emit()` compares only the files it is ABOUT TO WRITE, so
 * it can find a page that is wrong and never a page that should not exist.
 * Rename a declared graph's id, or drop the declaration, and its dashboard
 * survives at a URL nothing links to, with `--check` reporting clean over it.
 * That is `ankg`'s defect in this generator, and it could not take `ankg`'s
 * fix until a dashboard could say which graph it is for.
 *
 * `orphanSubjectPages` is that fix's helper, reused rather than reinvented —
 * the bean asked for exactly that. The only thing supplied here is how to
 * read a page's own claim, so **ownership is READ off the file** and the site
 * root stops being a hazard: a directory becomes a candidate by carrying a
 * marker that names ITSELF, never by sitting here.
 *
 * Two departures from `gen-schema-viz`, both because of where this publishes:
 *
 * 1. **The `foreign` half is not reported.** There, a neighbour in a tree of
 *    nothing but subjects is an anomaly worth a line. Here foreign is the
 *    ordinary case — every docs page, `assets/`, everything just-the-docs
 *    emits — and a line per neighbour on every run trains a reader to skip
 *    the place a real finding appears.
 * 2. **The generator's own FILE goes, not the directory.** A sibling in that
 *    directory is somebody else's, and a recursive remove would take it
 *    along. The directory follows only when nothing else was in it.
 *
 * @returns how many findings `--check` should count.
 */
export function pruneOrphanDashboards(
  siteDir: string,
  wanted: readonly string[],
  checkOnly: boolean,
): number {
  const { owned: orphans } = orphanSubjectPages(siteDir, wanted, dashboardOwner);
  let found = 0;
  for (const id of orphans) {
    const dir = join(siteDir, id);
    found++;
    if (checkOnly) {
      console.error(
        `  ✗ ${relative(ROOT, dir)} is an orphan — a dashboard for a state graph ` +
          `the declaration no longer describes`,
      );
      continue;
    }
    ...
    if (left.length === 0) {
      ...
      console.log(`  ✗ pruned ${relative(ROOT, dir)}`);
    } else {
      console.log(
        `  ✗ pruned ${relative(ROOT, dir)}/index.html — directory kept, ` +
          `${left.length} file(s) in it this generator did not write`,
      );
    }
  }
  return found;
...
stale += pruneOrphanDashboards(SITE, graphs.map((g) => g.id), check);

...
description: >
  Rendering only what changed — seeding a build from a previous render, the
  two edges that decide what must re-run, why a step declares GRAPH KINDS
  rather than paths, and the rule that could-not-determine always re-renders
  rather than serving a stale page.
---
...
| `inputGraphs` | graph **kinds** from the declaration — `cat-harness`, `beans`, `schemas`, … | anything the declaration already locates |
...
- [`directory-conventions`](directory-conventions.md) — what a graph kind is
  and where the declaration declares it.

---
name: graph-detanglement
description: >-
  Drive a tangled graph toward a clean partition, so a sub-graph can be split
  off when it grows too large or when a business reason separates it. The
  staged practice — declare in place, detangle, isolate, extract — with the
...
repository, still connected, nothing moved. `<name>.json` gains a directory
...
artefact. `cat-bootstrap/` is the demonstrated case: its own `<name>.json`, its

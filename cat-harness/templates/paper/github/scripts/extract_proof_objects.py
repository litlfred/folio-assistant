...
labels.  Outputs or updates `proof-objects.json` in the paper's build
directory (`build/<paper>/`; see `folio_lean/config.py`).

Usage:
    FOLIO_PAPER=<paper> python .github/scripts/extract_proof_objects.py [--output PATH]

The chapters are the ones the platform's `build.ts` wrote, in the order
`main.tex` inputs them. Nothing here lists a paper's chapters, and the Lean
file behind a `\\lean{}` declaration is found by searching the package.
...
from folio_lean.manifest import (
    ...
    while i < len(lines):
        ...
        if label:
            ...
            if lean_decl:
                ...
                lean_file = find_decl_file(lean_decl)
                if lean_file:
                    ...
                    if GITHUB_REPO:
                        src = (LEAN_DIR / lean_file).relative_to(REPO_ROOT)
                        lean_ref["github_source_url"] = (
                            f"https://github.com/{GITHUB_REPO}/blob/{commit}/{src}"
                        )
...
def main():
    ...
    # No chapters is not "no proof objects": it is a build that did not run,
    # and writing an empty manifest would report every object as gone.
    if not CHAPTER_FILES:
        print(f"❌ No chapters under {CHAPTERS_DIR}. Run the platform's build.ts first.")
        raise SystemExit(1)
    ...
    manifest = {
        ...
        "manuscript": {
            ...
            "lean_docs_url": f"{GH_PAGES_BASE}/lean/" if GH_PAGES_BASE else "",
        },
        "objects": all_objects,

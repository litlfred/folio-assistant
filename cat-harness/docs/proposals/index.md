...
| [Wallet custody](wallet-custody.html) | May a self-sovereign harness HOLD a person's credentials, not just produce what wallets consume? **Owner, 2026-09-24: it may.** Seven questions come before any code: whose credentials, hold or broker, where the bytes live (never the repository), which actors may use one, audit, offline revocation, and which standards. Issue #370 |
...
| [Which top-level directories are not declared subgraphs?](declaration-gap.html) | The first half of the owner's 2026-09-23 subgraph ruling — *"in harness `<stub>/*.*` should be a subgraph"* — measured rather than swept. 80 top-level directories across 17 instances, 62 declared, 18 not; but only **12 hold a file no declared directory covers**, and `cat-harness/content` is 718 of those 744 and already bean `ylj7`'s. **Outside it the whole gap is 11 directories and 26 files.** Each is classified declare / fold / remove / already-declared / not-mine. Four of the removal candidates are QOU folio content the partition left in the platform stub; `cat-harness/viewer/` is the sharp case, being folio-specific *and* e2e-tested. Records three corrections to an earlier count, including the `scope: "repository"` join that made eight declared directories look undeclared |
| [Translation support by block kind](translation-block-audit.html) | Which of the 16 block kinds does the gettext pipeline actually translate? **An audit: none is supported end to end**, because nothing renders a translated block. `prose` is partial. The math kinds are extracted, but their math is corrupted in the msgid (`$a_i$` becomes `$ai$`). Captions, titles and figure descriptions sit in the `.ts` manifest, and nothing extracts them. Injection deletes every blank line. Storage check: the per-locale `.po` files match the owner's model, but the `.pot` is copied into every target locale (363 templates against 19 catalogues) instead of kept as one source |

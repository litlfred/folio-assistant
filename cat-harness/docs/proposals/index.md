...
| [Actors, ODRL and PROV-O](odrl-prov-actor-model.html) | How to simplify the actor / role / permission schema, now that the owner has chosen **W3C ODRL 2.2** for permissions and **W3C PROV-O** for logging (2026-09-23)? Actors become `prov:Agent`s with no permission list. The 14 flat permission ids become an ODRL profile in which 9 are kinds of task under `folio:perform`. Grants are ODRL rules scoped by Process, Task and Role constraints, and every task run is a `prov:Activity`. The deterministic and the agentic BPMN engine then check one policy, before and after the fact. Computable data sharing agreements (`odrl:Agreement` as a VC, GDHCN, HCERT) are the next proposal |

---
title: "UML — folio-assistant-core/core-schemas"
nav_exclude: true
---

# UML — folio-assistant-core/core-schemas

Generated by `cat-harness/scripts/gen-uml-overview.ts` — do not edit. The `core-schemas` sub-graph of `folio-assistant-core` (`folio-assistant-core/schemas`), with every attribute read from its node schema.

**Sources (same model):** [PlantUML](https://github.com/litlfred/folio-assistant/blob/main/cat-harness/uml/overview/folio-assistant-core/core-schemas.puml) · [Mermaid](https://github.com/litlfred/folio-assistant/blob/main/cat-harness/uml/overview/folio-assistant-core/core-schemas.mmd)

```mermaid
classDiagram
  direction TB
  namespace folio_assistant_core__core_schemas {
    class n_folio_assistant_core_core_schemas_schemas["schemas: no node schema"] {
      <<could not determine>>
    }
    class n_folio_assistant_core_core_schemas_cat_harness["cat-harness: no node schema"] {
      <<could not determine>>
    }
  }
  cssClass "n_folio_assistant_core_core_schemas_schemas" fa_uml_kind_schemas
  cssClass "n_folio_assistant_core_core_schemas_schemas" fa_uml_undetermined
  cssClass "n_folio_assistant_core_core_schemas_cat_harness" fa_uml_kind_cat_harness
  cssClass "n_folio_assistant_core_core_schemas_cat_harness" fa_uml_undetermined
```

| sub-graph | directory | graph kinds | node schema |
|---|---|---|---|
| `folio-assistant-core/core-schemas` | `folio-assistant-core/schemas` | schemas, cat-harness | schemas: *could not determine*; cat-harness: *could not determine* |

## Sub-graphs

- [← all of folio-assistant-core](../folio-assistant-core.html)

...
> This is the entry; the visualiser is the [Themes page]({{ '/themes/' | relative_url }}),
> generated by `bun run theme:page` (and, as a standalone file, `bun run theme:sheet`).
...
## Looking at it — the [Themes page]({{ '/themes/' | relative_url }})

Published on this site at `themes/`, generated by `bun run theme:page` and
gated by `theme:page:check`, so it cannot describe themes that no longer exist.
`bun run theme:sheet` writes the same rows as one standalone file with the art
inlined, for opening offline or attaching to a review. Both go through one
renderer, so the page and the sheet cannot disagree.

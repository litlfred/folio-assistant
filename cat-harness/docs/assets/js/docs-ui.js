...
(function () {
  ...
  /* ═══ The work-plan dashboard ═════════════════════════════════════════
   *
   * What the bean store and the todo store currently HOLD, mounted into the
   * `beans-and-todos` page beside the prose that says what they ARE.
   *
   * ## Why this is not a second sticky board
   *
   * `mountTodoBoard` above renders one sticky per item, and that is right for
   * three todos. There were 239 beans when this was written. A wall of 239
   * stickies is not a board, it is a scroll, and the question a reader brings
   * to a work plan that size is "how much, where, and what is stuck" — three
   * aggregates, not 239 cards. So the same data gets a different surface, and
   * the sticky machinery is left alone rather than stretched to a scale it was
   * not designed for.
   *
   * ## The form of each panel follows the job of its data
   *
   *   - **Counts** are a headline. Four numbers, rendered as numbers. A bar
   *     chart of four bars restating the same four numbers is the commonest
   *     way a dashboard adds ink without adding information.
   *   - **Open beans per epic** is magnitude across identities whose names are
   *     long, which is what a horizontal bar is for. One series, so no legend:
   *     the heading names it. Every bar carries its own label and value as
   *     text, so the chart IS its own table view.
   *   - **Findings** are three or four rows. A list. Each row carries an icon
   *     and a word as well as its colour, because a status colour never
   *     carries meaning alone.
   *
   * ## Age is computed HERE, and that is a constraint from the generator
   *
   * `gen-docs-pages.ts` gates the projection on exact content, so a finding
   * computed against the clock would change the file on every build and the
   * staleness gate would fire forever. The projection therefore publishes
   * `updatedAt` as a fact and this page computes age at view time — which is
   * also the right way round: the page moves as time passes without the
   * repository moving.
   *
   * ## Everything reaches the DOM through textContent
   *
   * A bean's title and body are authored, travel through a JSON file and land
   * here, so the same rule the sticky board follows applies: no innerHTML
   * touches input. The only markup built as a string is the static glyph
   * constants, which no input reaches.
   */
  ...
  /** Fetch a published index by its `<meta>` name, or `null`. */
  ...
  function mountWorkPlan() {
    ...
    fetchIndex("fa-beans-src", function (beanDoc) {
      if (!beanDoc) return;                       // fallback prose stays put
      fetchIndex("fa-todo-src", function (todoDoc) {
        ...
        board.appendChild(countsPanel(beans, todoDoc ? todoDoc.items : null));
  ...
  function init() {
    ...
    paintQaBadges();
    mountTodoStickies();
    // The aggregate view of the same two stores, on the one page that
    // documents them. It mounts only where that page declares a container, so
    // every other page pays a single failed `querySelector` for it.
    mountWorkPlan();

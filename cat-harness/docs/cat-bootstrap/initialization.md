...
| | from |
|---|---|
| that this repository is **not yet** an instance | no root `<name>.json` |
| the instance to become one **of** | the single reference a person supplied |
| what that instance **is** | its own `<name>.json` |
...
1. **Write this repository's `<name>.json`** from what the upstream

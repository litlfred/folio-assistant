...
| `requirements` | **harness** | what the harness promises, one page per shipped feature: a proposal MOVED here on ship, its front matter a `Requirement` (`bootstrap/schemas/requirement.ts`), checked by `check:requirements`. A **sub-graph of `docs`**, declared from within. Issue #1164. | no — its pages are built by `docs` |

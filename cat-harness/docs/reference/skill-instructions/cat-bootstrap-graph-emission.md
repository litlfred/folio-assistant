...
{: .note }
> Generated from [`../cat-bootstrap/render/cat-bootstrap-graph-emission.md`](https://github.com/litlfred/folio-assistant/blob/main/../cat-bootstrap/render/cat-bootstrap-graph-emission.md) — do not edit here.
>
> [✎ Edit this page's source](https://github.com/litlfred/folio-assistant/edit/main/../cat-bootstrap/render/cat-bootstrap-graph-emission.md){: .fa-edit-source }

{% raw %}
# Emitting cat-bootstrap's own graph

**The exemption and this skill are one trade, not two facts.** cat-bootstrap
...
`cat-harness/scripts/gen-cat-bootstrap-graph.ts`, exposed as
`bun run cat-bootstrap:graph`. It resolves the directories to scan **from the
declaration**, never by walking the tree — which is the axis
`scripts/tests/cat-bootstrap-graph.test.ts` defends on purpose: the disk side
of each assertion *names* the directory, the exporter *resolves* it, and a
resolver that stops finding `cat-bootstrap/skills/` would otherwise export an
...
`omitted: [packages, registry, schemas, tools]`. **"cat-bootstrap has no tools"
...
Drop the `.md` in `cat-bootstrap/skills/` or `cat-bootstrap/render/`, add its
...
- [`cat-bootstrap-graph-publication`](cat-bootstrap-graph-publication.md) — where
  the document lands and why its `@id` must equal that path
- [`cat-bootstrap-kg-navigation`](../skills/cat-bootstrap-kg-navigation.md) —

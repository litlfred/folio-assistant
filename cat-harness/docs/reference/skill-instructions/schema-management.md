...
{: .note }
> Generated from [`skills/folio-core/schema-management.md`](https://github.com/litlfred/folio-assistant/blob/main/skills/folio-core/schema-management.md) — do not edit here.
>
> [✎ Edit this page's source](https://github.com/litlfred/folio-assistant/edit/main/skills/folio-core/schema-management.md){: .fa-edit-source }
...
So `viewer-prune.ts` prunes a candidate only when its own bytes identify it,
and anything else — a hand-authored page, a stray file, another tool's output
— is **reported and left**. The directory scopes the SEARCH; the file decides
the DELETE. `prunableStickies` in `ensure-landing-sticky.ts` is the shape this
reuses: its third filter keeps only candidates that parse as the thing it
writes.

**Two signatures, because the orphans predate the marker.** New pages carry
`viewerMarker(<generator>)`. Pages written before it do not — and an orphan is
never rewritten, so it would never acquire the marker and never become
prunable. A marker-only test could not remove the very file that motivated the
bean, so the legacy pair (`DATA_HREF` and `SCOPE`) closes the gap without a
migration.

The marker names its **generator**, not just "generated", so two generators
publishing under one root cannot each read the other's output as its own to
delete.

### Verify the PAIR, never one half

Bean `ankg` names both, because either alone proves nothing: a planted orphan
is pruned **and** a hand-authored sibling survives. A pruner that cleared the
directory passes the first and fails the second; one that deleted nothing
passes the second and fails the first.
`scripts/tests/viewer-prune.test.ts` asserts both, and the whole subject
*directory* goes rather than only its `index.html` — a page left as an empty
directory is still a URL that resolves.

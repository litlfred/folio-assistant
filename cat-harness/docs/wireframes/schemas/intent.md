...
All four are written by `cat-harness/scripts/gen-schema-viz.ts`. It is a zero-dependency viewer that fetches its projection (`assets/schemas/index.json`) relative to its own location. The drawing uses `cat-harness` (812 declarations, 122 modules, 517 edges, 8 undetermined). The others use the same layout with less data: bootstrap-tools 11 / 2 / 6, folio-assistant-core 60 / 11 / 31, large-datasets 11 / 2 / 1.

A fifth, `detangle`, was dropped 2026-09-23 when that instance was folded into cat-harness (bean `byql`); its two modules now render on the `cat-harness` page.

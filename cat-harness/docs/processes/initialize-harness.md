...
|---|---|---|---|
| **List the harnesses this could be**<br>`A_ListHarnesses` | Initiator | `confirm-harness` | Zero or more. The DEFAULT is bootstrap itself. Context may name others — "please initialize litlfred/folio-assistant" in a discussion is one, and so is a derivative such as cat-harness or smart-guidelines. This step narrows; it does not decide. |
| **List the locations this could install to**<br>`A_ListLocations` | Initiator | `confirm-harness` | Zero or more. Is this a git repository already, or were one or more URLs to repositories supplied? Both are Knowledge Graph Data Stores; the difference is only how they are reached. |

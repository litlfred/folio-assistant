...
**64** diagram(s), **64** parsed, **0** that would not load.

| where | diagrams |
|---|---|
| `cat-harness/processes/` | 52 |
...
**87** distinct skill(s) are named by an activity.

| skill | run by |
|---|---|
| `activity-log` | `activity-log.bpmn` |
| `adjudication` | `adjudication.bpmn`, `ingest-l1-completeness-gate.bpmn`, `refresh-materialized.bpmn`, `review-narrative.bpmn`, `translation-workflow.bpmn`, `voice-review.bpmn` |
...
| `content-feedback` | `adjudication.bpmn`, `content-lifecycle.bpmn`, `draft-to-publication.bpmn`, `review-narrative.bpmn` |
...
| `methodology-adoption` | `options-analysis.bpmn`, `diig-investment-path.bpmn` |
...
Every one of the **64** diagrams has a rendered SVG.

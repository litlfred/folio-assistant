...
| Work plan — beans (shared by humans and agents) | — | Where every failure branch lands before the process ends rather than just stopping: Gateway_Valid's `no`, Gateway_QcPass's `no` and Gateway_Restore's `environment error` each write here — Task_LogFindings, Task_QcBeans, Task_LogEnvError — before EndEvent_Findings, EndEvent_FindingsQc or EndEvent_EnvError, so a run that stops short always leaves a record of why rather than only a red check. |
...
| **SUSHI on the restricted tank**<br>`Task_Sushi` | Incremental build (system) | [`l3-fhir-authoring`](../reference/skill-instructions/l3-fhir-authoring.html) | Compile the restricted checkout with SUSHI — the cone's FSH, not the whole IG. A build optimisation, not a validation shortcut: SUSHI compiling is not conformance, so a green compile goes on to validation rather than standing in for it. |
...
| **Validate the cone (fhir_validate)**<br>`Task_Validate` | Publisher + validator services (JVM) | [`fhir-validation`](../reference/skill-instructions/fhir-validation.html) | Validate the cone's resources against the warm validator and report the cone that was checked. A validator that could not start is could-not-determine, never a pass with no findings. A green cone is not a green IG: a profile the change did not touch can still be broken by it. |
| **Log findings on the bean**<br>`Task_LogFindings` | Work plan — beans (shared by humans and agents) | [`todo-manager`](../reference/skill-instructions/todo-manager.html) | Validation failed: record the findings on the change's bean, so the work plan says what blocks it. Check before creating — beans create is not idempotent. |
...
| **Merge restored and rebuilt records**<br>`Task_Merge` | Incremental build (system) | [`ig-publication`](../reference/skill-instructions/ig-publication.html) | Combine the restored per-artefact records with the cone's freshly rendered ones into one complete set, so the meta-index rebuild that follows has every artefact's record and never re-derives one. |
...
| **Assemble the site (template · Jekyll)**<br>`Task_Site` | Incremental build (system) | [`ig-publication`](../reference/skill-instructions/ig-publication.html) | Assemble the site from the merged records: the IG template and a Jekyll build over output/. Only site assembly happens here — every artefact record already exists. |
...
| **File QC findings as beans**<br>`Task_QcBeans` | Work plan — beans (shared by humans and agents) | [`todo-manager`](../reference/skill-instructions/todo-manager.html) | QC failed on the aggregate: file each finding as a bean, so it is tracked on the work plan rather than lost in qa.json. Check for an existing bean before creating one. |

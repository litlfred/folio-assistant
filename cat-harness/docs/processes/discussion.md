...
|---|---|---|---|
| **Narrow the candidates from context**<br>`A_NarrowCandidates` | Initiator | `discussion` | Before asking. A repository whose root already holds a declaration, `<name>.json`, is not a blank slate, and a question whose candidates the agent could have worked out itself wastes the one question it is entitled to. |

...
| `bootstrap/bootstrap.json` | this instance's own declaration |
| `bootstrap/skills/` | the skills the processes name, plus the roles they act as — `skills/package-manifest.json` is the list |
| `bootstrap/workflows/` | ONE process: `initialize-harness.bpmn` |
...
2. **Find out whether this repository is already an instance.** It is if it has
   a `bootstrap.json` at its root declaring a `name`. If it has one, read it and
...
([`bootstrap.json`](bootstrap.json), role `agent-instructions`). It is copied into

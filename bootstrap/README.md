...
> `bun run check:declared-assets`. [`bootstrap.jsonld`](bootstrap.jsonld) is
> generated by `bun run bootstrap:graph` and **committed** — unlike `_kg/`,
> because its reader has nothing installed and cannot run a build — with CI
> gating it through `bootstrap:graph:check`. Still to come: the process
> diagram. Design and open questions:
...
1. **Load the skill for reading a knowledge graph** —
   ...
   step 2 without it.
2. **Load [`bootstrap.jsonld`](bootstrap.jsonld)** and read its `@graph`. The
   bootstrap **process** node is not in it yet — §3's decision tree is still to
   be built — so today the graph carries the two skills, this instance's
   declared directory and the graph kinds, and says in `omitted` what it did
   not look for.

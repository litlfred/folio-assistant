...
#   3. Per-block standalone PDF render for each changed block  [if pdflatex available]
...
# Only run if latexmk is available (local or Docker).
# Drive push happens automatically when GDRIVE_FOLDER_PATH is set or
# folio.config.json contains googleDrive.folderPath.

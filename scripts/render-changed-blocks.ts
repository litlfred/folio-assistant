 ...
 * render-on-change hook so Drive stays up to date after every content edit.
 *
 * Usage:
...
async function loadAllBlocks(): Promise<{
  ...
  const result = await buildPaper(paperManifestPath);
...
async function renderBlock(entry: BlockEntry, paper: any): Promise<BlockResult> {

...
<a class="skip" href="#list">Skip to results</a>
<header>
...
<main>
  ...
  <div class="list">
    ...
    <label class="visually-hidden" for="q">Search nodes by name, id or description</label>
...
function drawList() {
  ...
  el("count").textContent = hits.length === 1 ? "1 node matches" : hits.length + " nodes match";
  if (hits.length === 0) {
...
function neighbourhood(n, back) {
  ...
  // set of controls.
  let s = '<svg viewBox="0 0 ' + W + ' 300" role="group" aria-label="One-hop neighbourhood, ' +
  ...
  nodes.forEach((m, i) => {
    ...
    s += '<g class="node" role="button" tabindex="0" data-goto="' + escape(m.id) +
      '" aria-label="' + escape((m.dir === "in" ? "Referenced by " : "Links to ") + name + " via " + m.via) + '">';

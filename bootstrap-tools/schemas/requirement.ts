 ...
 *
 * Bootstrap holds no code (FR-7), so THIS file is the source and
 * `bootstrap/schemas/requirement.schema.json` is what bootstrap publishes —
 * generated by `bun run bootstrap-tools:schemas`, the same arrangement as the
 * declaration schema beside it. Zod's refinements do not survive the
 * conversion, so {@link REQUIREMENT_JSON_SCHEMA_CONDITIONALS} restates them
 * as JSON Schema, and `requirements.test.ts` proves the published document
 * refuses what this one refuses.
 *
 ...
 *
 * There is no `satisfiedBy`: what satisfies a statement points AT it, by
 * {@link requirementRef}, rather than the statement listing its satisfiers —
 * a statement is written once, satisfiers arrive later, and a list kept on
 * the statement would be edited each time one did.
 */
    ...
    properties: {
      statements: {
        items: {
          allOf: [
            {
              if: { properties: { kind: { const: "functional" } }, required: ["kind"] },
              then: { not: { required: ["category"] } },
            },
            {
              if: { properties: { kind: { const: "non-functional" } }, required: ["kind"] },
              then: {
                not: { anyOf: [{ required: ["activity"] }, { required: ["capability"] }, { required: ["benefit"] }] },
              },
            },
          ],
        },

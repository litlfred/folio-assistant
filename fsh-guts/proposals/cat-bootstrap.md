...
title: "The cat-bootstrap graph"
kind: proposal
movedOn: 2026-09-19
movedFrom: "docs/folio-assistant/proposals/cat-bootstrap.md"
summary: >-
  Design for `cat-bootstrap/` — the graph an agent reads before it knows whether the repository is an instance, what kind, or what for. NOTHING HERE IS BUILT. Arrived on main in docs/ the same day proposals left the rendered site; moved here by the same rule, since a design a folio's readers do not need is not a page. Bean `x3bd`.
---

# `cat-bootstrap/` — the graph an agent can read before it knows anything
{: .no_toc }
...
This sets out what cat-bootstrap is, what it deliberately excludes, and the two
...
- **A cat-bootstrap that cannot be read without MCP.** §4 forbids tool calls. If
  writing the two skills turns out to require one — to resolve a dependency, to
  read a remote declaration — then cat-bootstrap is not standalone and the
...
- **The size of the cached subset.** §6 assumes the named instance's
  ...
  answer is "most of cat-harness", cat-bootstrap is not lean and the cache is a
...
**`cat-bootstrap/` is its own knowledge graph, not a directory inside the harness
...
That inversion is the whole point. The reader of cat-bootstrap has not yet
established that cat-harness is present, let alone which version — so a
cat-bootstrap that imported from cat-harness could not run in the one situation it
...
link: **cat-bootstrap may not import harness.**
...
`cat-bootstrap/` is neither: it is a top-level directory of its own, because it is
what runs *before* there is an instance to name a stub after.

## 2. `README.md` is the content, and it is brief

The cat-bootstrap graph's documentation lives in `cat-bootstrap/README.md`, so it is
...
4. **It ends with a three-step handoff**, which is the README's actual job:
   ...
   them at the cat-bootstrap JSON-LD and have them load the graph and read the
   cat-bootstrap process; start the cat-bootstrap BPMN.
...
so cat-bootstrap's process is inspectable by the same tools without a second
mechanism.
...
**There is no MCP tool call anywhere in cat-bootstrap.** Tool calls are
cat-harness-level machinery; an agent in cat-bootstrap may not have a connected
server, and a cat-bootstrap that required one would fail in exactly the
cold-start case it exists for.

For the same reason, **UI concerns are not cat-bootstrap's**. Translations, the KG
viewer, the skills and tooling around them belong to cat-harness. And cat-bootstrap
must not need `VoiceGraph`, `LibraryGraph` or `PreviewGraph` — those are
content-model concepts that presuppose a folio, which cat-bootstrap does not yet
know it has.

**Hardcoded JSON/JSON-LD assets in cat-bootstrap are acceptable but not
preferred.** A cat-bootstrap that cannot resolve anything has to carry something;
...
**And it does not end at the cache.** The owner, 2026-09-19: cat-bootstrap's last
step, *after* installing cat-harness, is to **re-read the active harness's own
cat-bootstrap next-steps**. So the full order is:
...
does not yet have a harness, so it can only do what cat-bootstrap alone can do. The
...
cat-bootstrap that predates it. Hardcoding the post-install steps into cat-bootstrap
would make cat-bootstrap the authority on a graph it does not own, and would go
...
When cat-bootstrap instantiates a repository that is not yet an instance, the
upstream library is **replicated under `cat-bootstrap/<instance>/`** as a local
cache — `cat-bootstrap/cat-harness/`, and under §5 `cat-bootstrap/f-a-sci/` beside it —
and those directories are **declared in cat-bootstrap's own `harness.json`**, so
...
## 7. `AGENTS.md` is a declared asset of the cat-bootstrap instance

The owner, 2026-09-19: *"agents.md should itself be an asset as part of
cat-bootstrap"*, and *"cat-bootstrap should be an instance and contain its own
definitions — minimal, self-defining."*

§1 and §6 already have most of this: cat-bootstrap declares itself, and its cache
directories live in **cat-bootstrap's own `harness.json`**. Saying *instance*
...
Default behaviour is to copy `cat-bootstrap/AGENTS.md` into the repository being
initialized. That file does not exist yet; `cat-bootstrap/` currently holds
`README.md` alone.
...
would fail as `cat-bootstrap/AGENTS.md`.** Its cold-start line is
`scripts/install-beans.sh && … && beans prime`; its "Getting skills" box says
to use `skill_list` and `skill_fetch`. A repository mid-cat-bootstrap has no
`scripts/`, no `skills/`, no beans and no MCP server — and §4 forbids a tool
call anywhere in cat-bootstrap precisely because an agent here may have none.
...
a cat-bootstrap pointer, not the source of truth."* That sentence describes this
...
`f-a-sci`'s knowledge graph is not all of cat-harness, and a cat-bootstrap that
...
adjacent work that has shipped and that cat-bootstrap depends on:

| shipped | why cat-bootstrap needs it |
|---|---|
| `tools/<stub>/` with an aggregating barrel | the cache in §6 reuses this exact shape |
| `harness.json` directory declarations | cat-bootstrap declares its cache through them |
| `workflowDirs()` / `workflowFiles()` reading the declaration | a topical `cat-bootstrap/workflows/` is found without a code change |
| `bs:` / `cat:` / `fac:` namespaces | cat-bootstrap's terms are minted in its own namespace |
| `check:declared-paths` | enforces §4's "declared and checked" rule corpus-wide |

Still outstanding before cat-bootstrap can be built as described: `resolveSkillDirs`
...
**§7 is the part that does not wait on that.** Declaring the cat-bootstrap instance,
declaring `AGENTS.md` as its asset with a source ref, and authoring a
phase-1-valid `cat-bootstrap/AGENTS.md` are all statements about *cat-bootstrap's own*

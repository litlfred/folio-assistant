 ...
 *
 * @module cat-bootstrap-tools/schemas/discussion
 * @graphNode schema
 *
 * ## These shapes describe `cat-bootstrap`, and deliberately do not live in it
 *
 * `cat-bootstrap/README.md` promises an Initiator that it needs **no harness,
 ...
 * not know zod at all"*.
 *
 * `cat-bootstrap/skills/discussion.*.schema.json` is GENERATED from here. The
...
export const DiscussionOutputObjectSchema = z.object({
  ...
  harness: z
    ...
    .optional()
    .describe(
      "The harness this repository is to become — `cat-bootstrap`, or a derivative such as `cat-harness`, `folio-assistant`, `smart-guidelines`. Required when `outcome` is `settled` and `harness` was open.",
    ),
  repositories: z
    .array(RepositoryRefSchema)
    .optional()

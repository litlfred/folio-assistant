...
## Nothing you add here may make `cat-bootstrap` need a tool
...
So the direction of travel is one-way: **`cat-bootstrap-tools` reads and
writes `cat-bootstrap`; `cat-bootstrap` never imports from here.** If a change
would put an import, a dependency, a `package.json` or a build step into
`cat-bootstrap/`, it is the wrong change — the owner's words are *"bootstrap
should not know zod at all"*.

## The generated documents sit at a published `$id`, so what VALIDATES is a contract

`cat-bootstrap/skills/discussion.*.schema.json` is generated from
...
`bun test cat-bootstrap-tools/schemas/discussion.test.ts`. The `--check` twin

...
`cat-bootstrap/workflows/initialize-harness.bpmn` names `confirm-harness` **three
times**, and `cat-bootstrap/harness.json` declares the directory holding it.
...
at `cat-bootstrap/skills/` probes `cat-bootstrap/skills/workflows/`, a sibling of the real
directory), and **declared `cat-bootstrap/workflows/` at the root to fix it.**
...
- `skill-servable` ×3 — `confirm-harness` exists but no local package serves it,
  so `skill_fetch` answers "package not found". `cat-bootstrap/skills/` has no
  package manifest.
...
`test/results/kg-qa/cat-bootstrap/workflows/bootstrap.kg-qa.json` audits
...
> `skills/`, `bootstrap` at `cat-bootstrap/skills/`, `cat-harness-src` at
...
- [x] `cat-bootstrap/skills/` package manifest — **and the manifest was not the
...
- [~] the three unbound lanes — **NOT work, a recorded decision.** The four
      roles exist in `cat-bootstrap/skills/roles/roles.json` with the right flags
      ...
      **deliberate**. The file's own `_lanes_comment` records why: the root
      declares `cat-bootstrap/skills/` but NOT `cat-bootstrap/workflows/`, so a `lanes`
...
- [x] that diagram has no `BPMNDiagram` — **added 2026-09-20.** The premise
      ...
      the root does not declare `cat-bootstrap/workflows/`. Same isolation, again.

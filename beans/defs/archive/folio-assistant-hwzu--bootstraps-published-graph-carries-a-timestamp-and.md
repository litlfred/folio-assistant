...
updated_at: 2026-09-23T08:30:37Z
...
Owner ruled option 2, 2026-09-23: every published graph carries provenance.

- bootstrap-graph-emission: the "no timestamp, no commit SHA" row is narrowed to the `@graph` itself, with a new section explaining why. The old rationale ("a generated file cannot name its own commit") held while the document was committed. A published build names the commit it was DERIVED FROM (prov:wasDerivedFrom), which is correct.
- BootstrapGraphDocumentSchema: comment updated; `sourceCommitUnavailable` added.
- New test: the published copy carries generatedAt plus sourceCommit (or sourceCommitUnavailable), and no node in @graph carries any provenance key.

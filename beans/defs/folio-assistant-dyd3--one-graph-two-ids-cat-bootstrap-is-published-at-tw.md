...
title: 'ONE GRAPH, TWO @ids: cat-bootstrap is published at two paths with 84 subjects under two identities'
status: todo
type: task
...
`docs-site.yml` publishes the cat-bootstrap graph **twice**:

| path | written by | top-level fields |
|---|---|---|
| `<base>/cat-bootstrap.jsonld` | `kg-export --instance ./cat-bootstrap` | `generatedAt`, `sourceCommit*` |
| `<base>/cat-bootstrap/cat-bootstrap.jsonld` | `gen-cat-bootstrap-graph.ts` | `omitted`, `problems`, no timestamp |
...
will ever merge: a cold reader following `cat-bootstrap/README.md` lands on
...
- `gen-cat-bootstrap-graph.ts` exists for a reader "pointed at a repository
  with nothing installed", and its `@id` contract is the `cat-bootstrap/`
...
- **A** — collapse to the site root; make `cat-bootstrap/cat-bootstrap.jsonld`
  an alias. Cleanest graph; reopens the cold-start argument.
- **B** — collapse under `cat-bootstrap/`; repoint `skillHome`. Preserves the

...
updated_at: 2026-09-24T19:31:54Z
...
- [x] PROV activity written per verdict (#1180 step 5); GitHub login to actor mapping in the data store
...
Four PRs, all under issue #1207:
- #1214: every BPMN task runs the four-check `authorizeTask` (authenticated, assigned, authorized, access); `rbac.ts` on ODRL.
- #1289: `auth_whoami` (Tool node `user-auth`) asks GitHub who the caller is; GitHub's personal-account levels ARE the login mapping (owner / collaborator / viewer), by owner ruling; GitHub's strengths and weaknesses as the auth layer recorded in the skill.
- #1313: the engine is strict. `perform-task` for `owner` and `collaborator` in every lane ("all write roles collapse"); the principal is the one GitHub vouches for. Pinned: 628 tasks/gateways, owner and collaborator 628 allowed, viewer and nobody 0.
- this PR: the engine writes a `prov:Activity` per step as it records it (`src/workflow/prov-record.ts`); the PROV-O after-check reads it and re-checks the recorded principal, not a typed name.

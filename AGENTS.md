...
are in [`workflow-methodologies/crdm/crdm-detect.md`](cat-harness/workflow-methodologies/crdm/crdm-detect.md);
the full six-phase process is in
[`workflow-methodologies/crdm/crdm-requirements-workflow.md`](cat-harness/workflow-methodologies/crdm/crdm-requirements-workflow.md).

**The CRDM process is executable — do not hand-roll a phase tracker.**
`workflow-methodologies/crdm/workflows/crdm-requirements.bpmn` loads like every other diagram here,
...
| rule | where it lives |
|---|---|
| feature work is linked to a GitHub issue — scan before creating one, and never create one without permission | [`crdm-detect`](cat-harness/workflow-methodologies/crdm/crdm-detect.md) §"Issue association" |
...
| explicit user confirmation before merging to `main` | [`crdm-requirements-workflow`](cat-harness/workflow-methodologies/crdm/crdm-requirements-workflow.md) §"Phase 6 — Iterative development" |
| a round summary on the ISSUE, not only on the PR — **after each round**, not only at merge | [`issue-working`](cat-harness/skills/folio-core/issue-working.md), [`crdm-requirements-workflow`](cat-harness/workflow-methodologies/crdm/crdm-requirements-workflow.md) |
